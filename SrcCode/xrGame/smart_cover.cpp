...
cover::cover(smart_cover::object const& object, DescriptionPtr description, bool const& is_combat_cover) :
	inherited(object.Position(), object.ai_location().level_vertex_id()), m_object(object), m_description(description),
	m_id(m_object.cName()), m_is_combat_cover(is_combat_cover)
{
	m_is_smart_cover = 1;
	...
	for (; I != E; ++I, ++i)
	{
		...
		VERIFY2(graph.valid_vertex_id(level_vertex_id), make_string("invalid vertex id: smart cover[%s], loophole [%s]",
			m_object.cName().c_str(), (*I)->id().c_str()));
		vertex(**I, (*i).second);
...
void cover::vertex(smart_cover::loophole const& loophole, smart_cover::loophole_data& loophole_data)
{
	...
	VERIFY2(graph.valid_vertex_id(loophole_data.m_level_vertex_id), make_string("invalid vertex id: smart cover[%s], loophole [%s]",
...
u32 const& cover::action_level_vertex_id(smart_cover::loophole const& loophole, shared_str const& action_id) const
{
	...
	loophole_data::ActionVertices::const_iterator found2 = std::find_if(found->second.m_action_vertices.begin(),
...
bool cover::loophole_path(shared_str const& source_raw, shared_str const& target_raw) const
{
	...
	VERIFY2(result, make_string("failde to find loophole path [%s]->[%s] in cover [%s]", source.c_str(),
		target.c_str(), m_description->table_id().c_str()));
...
void cover::check_loopholes_connectivity() const
{
	...
	for (; I != E; ++I)
	{
		...
		VERIFY2(loophole_path(lhs, exit), make_string("failed to find path [%s -> %s] in smart_cover [%s]",
