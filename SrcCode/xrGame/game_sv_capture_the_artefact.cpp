...
void game_sv_CaptureTheArtefact::OnGiveBonus(KILL_RES KillResult, game_PlayerState* pKiller, game_PlayerState* pVictim, KILL_TYPE KillType,
...
void game_sv_CaptureTheArtefact::OnPlayerHitPlayer_Case(game_PlayerState* ps_hitter, game_PlayerState* ps_hitted,
	SHit* pHitS)
{
	...
	*/
	...
	if (childArtefactTeam != te)
	{
		...
		m_server->Process_event_reject(P, m_server->GetServerClient()->ID, 0,
			childArtefactTeam->second.artefactOwner->ID, childArtefactTeam->second.artefact->ID, true);
		*/
	}
...
BOOL game_sv_CaptureTheArtefact::OnTouch(u16 eid_who, u16 eid_target, BOOL bForced)
{
	...
	//VERIFY2(e_who, make_string("no actor (id = %id) touches the target (id = %d)").c_str());
	...
	*/
	...
	if (artefactOfTeam != te)
	{
		...
		/*
		if (std::find_if(teams.begin(), te, std::bind2nd(SearchOwnerIdFunctor(), e_who->ID)) != te)
		{
			return FALSE;
		}
		*/
...
void game_sv_CaptureTheArtefact::OnDetach(u16 eid_who, u16 eid_target)
{
	...
	/*
	TeamsMap::iterator artefactOfTeam = std::find_if(teams.begin(), te, std::bind2nd(SearchArtefactIdFunctor(), eid_target));
	*/
...
void game_sv_CaptureTheArtefact::CheckForArtefactReturning(u32 currentTime)
{
	...
	for (TeamsMap::iterator ti = teams.begin(); ti != te; ++ti)
	{
		...
		{
			/*
			if ((currentTime - ti->second.activationArtefactTimeStart) >= Get_ActivatedArtefactRetTime_msec())
			{
			*/
		...
		if (!ti->second.freeArtefactTimeStart || ((currentTime - ti->second.freeArtefactTimeStart) >=
