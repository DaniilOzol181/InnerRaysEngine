...
void CCoverManager::remove_nearby_covers(smart_cover::cover const& cover, smart_cover::object const& object) const
{
	...
	for (; I != E; ++I)
	{
		...
		m_nearest.erase(std::remove_if(m_nearest.begin(), m_nearest.end(), smart_cover::predicate(object)),
			m_nearest.end());
...
CCoverManager::Cover* CCoverManager::smart_cover(shared_str const& cover_id) const
{
	...
	SmartCovers::iterator found = std::lower_bound(m_smart_covers.begin(), m_smart_covers.end(), cover_id,
		id_predicate_less());
