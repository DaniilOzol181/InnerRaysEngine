...
void default_behaviour_planner::add_evaluators()
{
	add_evaluator(eWorldPropertyPlannerHasTarget, xr_new<evaluators::loophole_planner_const_evaluator>(&object(),
		"default behaviour planner has target", false));
	add_evaluator(eWorldPropertyLoopholeCanStayIdle, xr_new<evaluators::is_action_available_evaluator>(&object(),
		"can stay idle", "idle"));
	...
	add_evaluator(eWorldPropertyReadyToIdle, xr_new<evaluators::idle_time_interval_passed_evaluator>(&object(),
		"stay idle", object().default_idle_interval()));
}
