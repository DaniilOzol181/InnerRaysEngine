...
s16	game_cl_Deathmatch::GetBuyMenuItemIndex(u8 Addons, u8 ItemID)
{
	...
	/*
	if (SlotID == OUTFIT_SLOT)
	{
		SlotID = APPARATUS_SLOT;
	}
	*/
...
void game_cl_Deathmatch::SetBuyMenuItems(PRESET_ITEMS* pItems, BOOL OnlyPreset)
{
	...
	if (pCurActor)
	{
		...
		/*
		std::for_each(add_ammo.begin(), add_ammo.end(), std::bind1st(std::mem_fun<void, game_cl_Deathmatch, aditional_ammo_t::value_type const &>(
					&game_cl_Deathmatch::AdditionalAmmoInserter), this));
		*/
