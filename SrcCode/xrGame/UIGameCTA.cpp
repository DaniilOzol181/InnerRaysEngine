...
void CUIGameCTA::TryToDefuseGrenadeLauncher(CWeaponMagazinedWGrenade const* weapon, TIItemContainer const& all_items, aditional_ammo_t& dest_ammo)
{
	...
	R_ASSERT2(ammo_elapsed <= 1, make_string("weapon [%s] can't have more than one grenade in grenade launcher",
...
void CUIGameCTA::SetPlayerItemsToBuyMenu()
{
	...
	if (actor && !ps->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD))
	{
		...
		/*
		std::for_each(actor->inventory().m_slots.begin(), actor->inventory().m_slots.end(),
			std::bind1st(std::mem_fun<void, CUIGameCTA, CInventorySlot const &>(&CUIGameCTA::BuyMenuItemInserter), this));

		std::for_each(actor->inventory().m_belt.begin(), actor->inventory().m_belt.end(),
			std::bind1st(std::mem_fun<void, CUIGameCTA, PIItem const &>(&CUIGameCTA::BuyMenuItemInserter), this));

		std::for_each(actor->inventory().m_ruck.begin(), actor->inventory().m_ruck.end(),
			std::bind1st(std::mem_fun<void, CUIGameCTA, PIItem const &>(&CUIGameCTA::BuyMenuItemInserter), this));

		std::for_each(add_ammo.begin(), add_ammo.end(),
			std::bind1st(std::mem_fun<void, CUIGameCTA, aditional_ammo_t::value_type const &>(&CUIGameCTA::AdditionalAmmoInserter), this));
		*/
...
void CUIGameCTA::ShowBuySpawn(s32 spawn_cost)
{
	...
	sprintf_s(pay_frm_str, pay_frm_size, format_str, abs(Game().local_player->money_for_round),
		abs(spawn_cost));
...
void CUIGameCTA::LoadTeamDefaultPresetItems(const shared_str& caSection)
{
	...
	// “еперь дл€ каждое им€ оружи€, разделенные зап€тыми, заносим в массив
