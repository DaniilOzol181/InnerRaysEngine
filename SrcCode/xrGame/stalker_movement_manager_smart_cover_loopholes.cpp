...
float stalker_movement_manager_smart_cover::enter_path(LoopholePath* result, Fvector const& position, u32 const level_vertex_id,
...
float stalker_movement_manager_smart_cover::exit_path_weight(u32 const& source_node, Fvector const& source_position, u32 const& target_node,
...
stalker_movement_manager_smart_cover::transition_action const& stalker_movement_manager_smart_cover::nearest_action(smart_cover::cover const& cover,
	shared_str const& loophole_id0, shared_str const& loophole_id1, Fvector const& position, Fvector& result_position,
	u32& result_vertex_id, EBodyState* target_body_state) const
{
	...
	VERIFY2(result, make_string("cover[%s][%s], loophole[%s], body_state[%d] [%f][%f][%f]", cover.id().c_str(), cover.description()->table_id().c_str(),
		loophole_id0.c_str(), loophole_id1.c_str(), target_body_state ? *target_body_state : -1, VPUSH(position)));
	return (*result);
}

stalker_movement_manager_smart_cover::transition_action const& stalker_movement_manager_smart_cover::action(smart_cover::cover const& cover,
	shared_str const& loophole_id0, shared_str const& loophole_id1) const
{
	typedef smart_cover::description::TransitionGraph::CEdge edge_type;
	typedef smart_cover::description::ActionsList ActionsList;
	typedef smart_cover::transitions::action action;
...
void stalker_movement_manager_smart_cover::build_exit_path_to_cover()
{
	...
	for (; I != E; ++I)
	{
		...
		buffer_vector<shared_str> temp(_alloca(sizeof(u32) * m_temp_loophole_path.size()),
			m_temp_loophole_path.size(), m_temp_loophole_path.begin(), m_temp_loophole_path.end());
...
{
	VERIFY2(m_current.cover(), make_string("[%s][%s] -> [%s][%s], [%d]", m_current.cover() ? m_current.cover()->id().c_str() : "<world>",
		m_current.cover() ? m_current.cover_loophole()->id().c_str() : "<no loophole>", m_target.cover() ? m_target.cover()->id().c_str() : "<world>",
